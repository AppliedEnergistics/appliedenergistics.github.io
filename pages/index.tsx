...
import { getGuideVersions, GuideVersion } from "../lib/guides";

const GithubIcon = (
  <Icon path={mdiGithub} color="#181717" style={{  maxWidth: "1em", maxHeight: "1em"  }} />
);

const ModrinthIcon = <svg style={{  maxWidth: "1em", maxHeight: "1em"  }}>
  <use href="#modrinth-icon" />
</svg>;

const CurseforgeIcon = <svg style={{ maxWidth: "1em", maxHeight: "1em" }}>  <use href="#curseforge-icon" />
</svg>;
...
function ForgeRelease({
  ...
  return <>
    {release.modVersion}
    {release.curseforgePage && CurseforgeIcon  }
    {release.modrinthPage && ModrinthIcon}
    {release.githubReleasePage && GithubIcon}
  </>;
}
...
C-1987.4,866.2-1988.1,867.6-1988.9,868.1z"
      ...
      <div className="content">
        <p>
          You can download AE2 for various Minecraft versions from CurseForge,
          GitHub and Modrinth.
        </p>
        <p>
        ...
        <table>
          ...
          <tbody>
          {latestReleases.map(({ minecraftVersion, fabric, forge , guide}) => {
            return (
      ...
      <div className="downloadButtons">
        ...
        <a
          className="button is-large"
          href="https://modrinth.com/mod/ae2/versions"
        >
          <span className="icon">
            <svg>
              <use href="#modrinth-icon" />
          ...
          <span>Download from Modrinth</span>
        </a>
...
export const getStaticProps: GetStaticProps<{
  ...
  const guideVersions = keyBy(await getGuideVersions(), version => version.minecraftVersion);
  ...
  const latestReleases = Object.entries(latestByMcAndModLoader).map(
    ([minecraftVersion, releasesByLoader]) => {
      ...
      const guide = guideVersions[minecraftVersion];
  ...
  latestReleases.sort((a, b) => {
    const aVersion = semver.coerce(a.minecraftVersion) ?? "0.0.0";
    const bVersion = semver.coerce(b.minecraftVersion) ?? "0.0.0";
