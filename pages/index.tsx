...
import { getConsolidatedReleases } from "../lib/releases";
import { Release } from "../lib/releases/Release";

const GithubIcon = (
  <Icon path={mdiGithub} color="#181717" style={{ width: "1em" }} />
);

function Index({ releases }: InferGetStaticPropsType<typeof getStaticProps>) {
  return (
    <>
      <svg
      ...
      <div className="content">
        ...
        <ul>
          {releases.map((release) => {
            return (
              <li key={release.modVersion}>
                <b>{release.minecraftVersions}</b> {release.modVersion}
                {release.githubReleasePage && GithubIcon}
                {release.curseforgePage && (
                  <svg style={{ height: "1em", width: "1em" }}>
                    <use href="#curseforge-icon" />
                  </svg>
                )}
                {release.modrinthPage && (
                  <svg style={{ height: "1em", width: "1em" }}>
...
export const getStaticProps: GetStaticProps<{
  ...
  return { props: { releases: getConsolidatedReleases() } };
};
